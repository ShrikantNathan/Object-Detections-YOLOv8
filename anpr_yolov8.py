...
def force_resize_high_res_frames(frame: np.ndarray):
    # frame_resz: np.ndarray = np.array([])
    ...
    elif np.bitwise_and(np.equal(frame.shape[1], 1920), np.equal(frame.shape[0], 1080)):
        frame_resz = cv2.resize(frame, (1920, 1080), interpolation=cv2.INTER_LINEAR)
    else:
...
def run_instance_segmentation_inside_videos():
    ...
    video4 = np.random.choice([cv2.VideoCapture(r"C:\Users\ShrikantViswanathan\Videos\CityDrive-New.mp4"),
                               cv2.VideoCapture(r"C:\Users\ShrikantViswanathan\Videos\CityDrive-New.mp4"),
                               cv2.VideoCapture(r"C:\Users\ShrikantViswanathan\Videos\CityDrive2.mp4")])
    ...
    while True:
        ...
        frame_seg = cv2.resize(force_resize_high_res_frames(frame5), dsize=(0, 0), fx=0.5, fy=0.5)
        frame2_seg = cv2.resize(force_resize_high_res_frames(frame6), dsize=(0, 0), fx=0.5, fy=0.5)
        frame3_seg = cv2.resize(force_resize_high_res_frames(frame7), dsize=(0, 0), fx=0.5, fy=0.5)
...
def annotate_objects_inside_videos():
    ...
    video2 = cv2.VideoCapture(os.path.join(os.getcwd(), "Scene", "BusStopArm_Video", video_scenes, 'Cam_3.mp4'))
    video3 = cv2.VideoCapture(os.path.join(os.getcwd(), "Scene", "BusStopArm_Video", video_scenes, "Cam_4.mp4"))
    video4 = cv2.VideoCapture(r"C:\Users\ShrikantViswanathan\Videos\CityDrive-New.mp4")
    vid_writer = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'mp4v'), 22, (1920, 1080))
    ...
    while True:
        ...
        frame4: np.ndarray = cv2.resize(frame4, dsize=(0, 0), fy=0.5, fx=0.5, interpolation=cv2.INTER_NEAREST)
        print(frame4.shape)
        ...
        for res2 in lpr_results2:
            for box2 in res2.boxes.boxes:
                x2, y2, w2, h2 = int(box2[0]), int(box2[1]), int(box2[2]), int(box2[3])
                cropped_image2 = frame2[y2: h2, x2: w2]
                img_bytes2 = cv2.imencode(".jpg", cropped_image2)[1].tobytes()
                response = client.detect_text(Image={"Bytes": img_bytes2})
                main_text2 = str()
                textDetections2 = response["TextDetections"]
                for text in textDetections2:
                    if text["Type"] == "LINE":
                        main_text2 = main_text2 + " " + text["DetectedText"]
                print(f'Main Text: {main_text2}')

        print("FRAME -> 3 :: RESULTS")
        for res3 in lpr_results3:
            for box3 in res3.boxes.boxes:
                x3, y3, w3, h3 = int(box3[0]), int(box3[1]), int(box3[2]), int(box3[3])
                cropped_image3 = frame2[y3: h3, x3: w3]
                img_bytes3 = cv2.imencode(".jpg", cropped_image3)[1].tobytes()
                response = client.detect_text(Image={"Bytes": img_bytes3})
                main_text3 = str()
                textDetections3 = response["TextDetections"]
                for text in textDetections3:
                    if text["Type"] == "LINE":
                        main_text3 = main_text3 + " " + text["DetectedText"]
                print(f'Main Text: {main_text3}')

        print("FRAME -> 4 :: RESULTS")
        for res4 in lpr_results4:
            for box4 in res4.boxes.boxes:
                x4, y4, w4, h4 = int(box4[0]), int(box4[1]), int(box4[2]), int(box4[3])
                cropped_image4 = frame4[y4: h4, x4: w4]
                img_bytes4 = cv2.imencode(".jpg", cropped_image4)[1].tobytes()
                response = client.detect_text(Image={"Bytes": img_bytes4})
                main_text4 = str()
                textDetections4 = response["TextDetections"]
                for text in textDetections4:
                    if text["Type"] == "LINE":
                        main_text4 = main_text4 + " " + text["DetectedText"]
                print(f"Main Text: {main_text4}")
